    ...
    return {
        ...
        controller: function($scope, $element, $attrs, $location, dialogs){
			...
			$scope.clickMetric = function(idMetric, title, issued) {
				...
				//for (var k in $scope.metricsList)
				{
					//console.log("id="+$scope.metricsList[k].id+"--idMetric="+idMetric);
        ...
        '<div ng-show="datasetsList.length<numberMaxDatasets">'+
			...
			'<div class="filterDatasetPaginationBody createvisualization" id="filterDatasetsPaginationBodyDirective">' +
				'<ul class="datasets-list">' +
        			...
        			'</li>' +
        		'</ul>' +        		
				'<div class="filterDatasetsPagination" id="filterDatasetsPaginationDirective">' +
					'<button ng-show="datasetsFilter.hits.total>1" ng-disabled="pagToSearch==1"  class="btn btn-warning" ng-click="findDatasetsByFilter(\'prev\')">' +
						'<span class="glyphicon glyphicon-chevron-left"></span>  Previous' +
					'</button>' +
					'<button ng-show="datasetsFilter.hits.total>1" ng-disabled="datasetsFilter.hits.total<=pagToSearch*itemsperpagesize" class="btn btn-warning pull-right" ng-click="findDatasetsByFilter(\'next\')">' +	
						'Next <span class="glyphicon glyphicon-chevron-right"></span>' +
					'</button>' +
				'</div>' +		
			'</div>' +
